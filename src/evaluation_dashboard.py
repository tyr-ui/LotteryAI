...
def render_evaluation_dashboard_markdown(dashboard:Mapping[str,object])->str:
    ...
    for g in GAME_KEYS:
        ...
        stats=_map(game.get("statistical_evaluation")); paired=_map(stats.get("paired_evaluation")); operational=_map(stats.get("operational_evaluation")); interval=_map(paired.get("confidence_interval_95")); perm=_map(paired.get("permutation_p_value_reference")); sign=_map(paired.get("sign_test_p_value_reference"))
        lines += ["### 統計評価","",f"- 判定: {paired.get('judgement')}",f"- 指標: {stats.get('metric')}",f"- 比較基準: {stats.get('baseline')}",f"- データ量: {paired.get('data_volume')}",f"- 平均差: {_display_value(paired.get('mean_difference'))}",f"- 95%CI: {_display_value(interval.get('lower'))} ～ {_display_value(interval.get('upper'))}",f"- 平均差のpermutation p値（参考）: {_display_value(perm.get('value'))}",f"- 勝敗の符号検定p値（参考）: {_display_value(sign.get('value'))}",f"- 実運用評価: {operational.get('evaluated_draws',0)}回",f"- 実運用評価開始日: {operational.get('evaluation_started_at') or operational.get('started_at') or '未記録'}",""]
